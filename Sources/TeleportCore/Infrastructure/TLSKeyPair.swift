...
///
/// `nonisolated` (not MainActor): the keypair is constructed and consumed
/// from nonisolated contexts (the keypair generator protocol, the mocks'
/// fixture factories), and it holds no actor-protected state.
nonisolated public struct TLSKeyPair {
    ...
    /// Explicit `nonisolated` init: the implicit memberwise init is
    /// MainActor-isolated under `.defaultIsolation(MainActor.self)`, but the
    /// keypair is constructed from nonisolated contexts (the mock's fixture
    /// factory, the keypair generator protocol).
    public init(privateKey: SecKey, publicKeyPEM: String) {
    ...
    public var tlsPubKeyB64: String {
