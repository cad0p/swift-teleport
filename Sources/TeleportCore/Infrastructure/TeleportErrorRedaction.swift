...
//
//  Two deliberate carve-outs remain host-parity and are NOT wire payloads:
//  the connect-step `String(describing: error)` in
//  `TeleportRegistrationCoordinator` and the issued-cert `validityWindow`
//  timestamps in `TeleportIssuedCertValidator.Failure` (logged as
//  `failure.errorDescription` by the bootstrap/login coordinators). They carry
//  no server body or secret; their descriptive text is the triage signal.
//
...
package enum TeleportErrorRedaction {
    ...
    package static func grpcFailure(_ error: Error) -> String {
    ...
    package static func wireFailure(_ error: Error) -> String {
