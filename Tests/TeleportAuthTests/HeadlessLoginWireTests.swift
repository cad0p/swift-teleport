...
/// response.
nonisolated final class LoopbackHTTPServer:  Sendable {
    ...
    private nonisolated(unsafe) var captured: CapturedRequest?
...
nonisolated final class HeadlessLoginRecordingProtocol: URLProtocol {
    ...
    nonisolated(unsafe) static var scriptedError: Error?
    ...
    /// Sets the scripted error under the lock. A synchronous helper so
    /// async test methods do not call `NSLock` directly (unavailable from
    /// async contexts under Swift 6).
    nonisolated static func setScriptedError(_ error: Error?) {
        lock.lock()
        scriptedError = error
        lock.unlock()
    }
...
nonisolated final class HeadlessLoginWireTests: XCTestCase {
    ...
    func testPost_injectedSessionTimeoutSurfacesURLErrorCode() async throws {
        ...
        HeadlessLoginRecordingProtocol.setScriptedError(URLError(.timedOut))
    ...
    func testPostSignature_defaultsToTheSharedTrustSessionExpression() throws {
        ...
        let source = try String(
            contentsOf: repositoryRoot
                .appendingPathComponent("Sources/TeleportCore/Infrastructure/HeadlessLogin.swift"),
            encoding: .utf8
        )
        XCTAssertTrue(
