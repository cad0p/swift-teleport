...
//
//  Package port note (Phase 1b): imported byte-for-byte from
//  `VVTermTests/Features/Teleport/HeadlessIDTests.swift` (host commit
//  291d75fb); only the import and the isolation annotations changed.
//
...
// target's `.defaultIsolation(MainActor.self)` cannot apply to the class. The
// test methods opt back in with `@MainActor` (Swift 6 language mode).
nonisolated final class HeadlessIDTests: XCTestCase {
