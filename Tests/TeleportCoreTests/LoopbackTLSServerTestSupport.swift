...
enum LoopbackTLSServerTestSupport {
    ...
    static func fixtureURL(_ relativePath: String) -> URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()          // Tests/TeleportCoreTests
...
final class LoopbackTLSServer {
    ...
    init(identity: SecIdentity, alpnProtocols: [String]) throws {
        ...
        listener.newConnectionHandler = { [weak self] connection in
            // Hop to the main actor: the class is MainActor-isolated (the test
            // target's `.defaultIsolation`), and this handler is `@Sendable`
            // (Network.framework calls it on its own queue).
