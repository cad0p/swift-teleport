...
//  and its next read fails with `EBADF` (observed in this package's parallel
//  CI). All six paths now route through the shared `PumpFDCloser`.
//
//  XCTest (the host target also runs Swift Testing): this is the XCTest
//  counterpart of the removed Swift Testing cases, adapted to the transport's
//  `closeOnce(_:)` API. XCTest is required for the SIGPIPE case so
//  `continueAfterFailure = false` halts before the signalling write; a Swift
//  Testing `#expect` would record a failure and then raise SIGPIPE, killing
//  the test host.
//
...
nonisolated final class SSHTLSTransportPumpFDCloserTests: XCTestCase {
    ...
    /// (`Tests/TeleportCoreTests/SSHTLSTransportPumpFDCloserTests.swift`).
