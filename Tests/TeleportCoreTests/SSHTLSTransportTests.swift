...
struct SSHTLSTransportTests {
    ...
    @Test
    func loopbackTransportFailureThrowsTeleportPackageError() async throws {
        // The package transport throws `TeleportPackageError.connectionFailed`
        // when the loopback TLS handshake fails. The host maps that error
        // across the seam so the app's `error as? SSHError` classification
        // (disconnect-before-retry) still fires; that mapping is host-side
        // and covered there.
        let identity = try LoopbackTLSServerTestSupport.identity(named: "self-signed.p12")
        let server = try LoopbackTLSServer(
            identity: identity,
        ...
        #expect(packageError?.errorDescription == "Connection failed: \(message)")
    }
